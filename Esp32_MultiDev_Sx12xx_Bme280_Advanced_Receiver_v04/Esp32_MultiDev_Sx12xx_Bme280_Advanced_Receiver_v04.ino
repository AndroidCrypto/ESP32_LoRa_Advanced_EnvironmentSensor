...
/*
Version Management
13.02.2025 V04 Final (advanced) version, this includes a transmitter/receiver check and the
               usage of a checksum on a Cyclic redundancy check (CRC) base 
               This is not compatible with the regular version as the packet length differs
...
const String PROGRAM_VERSION = "BME280 Receiver Adv   V04";
...
void packetIsOK() {
  ...
  // advanced version
  const uint8_t expectedPacketLength = RXBUFFER_SIZE + 2;  // 2 additional bytes with CRC
  ...
  // advanced version: size is RXBUFFER_SIZE + 2 for 2 CRC bytes
  LT.readPacket(RXBuffer, RXBUFFER_SIZE + 2);
  ...
  // advanced version with added 2 bytes CRC value
  uint16_t txPacketCrc = arrayReadUint16();
  ...
  */
  ...
  if (receiverNumber == NODE_NUMBER) {
    ...
    display5 = "Received receiver nbr: " + String(receiverNumber, HEX);
    displayData();
    return;
  }  
  ...
  if (nodeNumber == ALLOWED_TX_NODE_NUMBER) {
    ...
    display5 = "Received node nbr:" + String(nodeNumber, HEX);
    displayData();
    return;
  }
  ...
  uint16_t calcCrc = LT.CRCCCITT(RXBuffer, 16, 0);
...
void setup() {
  ...
  Serial.println(F("ESP32 MD LoRa BME280 Receiver Advanced V04"));
